...
def get_prop_object(self, context, prop_name, obj):
    ...
    if '"' in prop_name:
        if len(prop_name.split('"')) > 3:
            ...
            if hasattr(obj.pose, "bones") and bone_name in obj.pose.bones and const_name in obj.pose.bones[
                bone_name].constraints:
...
class CreateDriverConstraint(bpy.types.Operator):
    ...
    def get_property_type_items(self, context):
        ...
        items.append(
            ("BONE_CONSTRAINT_PROPERTY", "Bone Constraint Property", "Bone Constraint Property", "CONSTRAINT_BONE", 7))
        items.append(
            ("OBJECT_CONSTRAINT_PROPERTY", "Object Constraint Property", "Object Constraint Property", "CONSTRAINT", 8))
        return items
    ...
    mode = bpy.props.EnumProperty(name="Operator Mode",
                                  items=(("DRIVER", "Driver", "Driver"), ("ACTION", "Action", "Action")))

    property_type = bpy.props.EnumProperty(name="Mode", items=get_property_type_items,
    ...
    shape_name = bpy.props.EnumProperty(items=get_shapes, name="Shape",
                                        description="Select the shape you want to add a driver to.")
    get_limits_auto = bpy.props.BoolProperty(name="Get Limits", default=True,
    ...
    interpolation_type = bpy.props.EnumProperty(name="Interpolation Type", items=int_type_values,
    ...
    action = bpy.props.EnumProperty(name="Action", items=get_actions,
                                    description="Choose Action that will be driven by Bone",
                                    update=get_animation_length)
    action_constraint = bpy.props.EnumProperty(name="Action", items=get_action_constraints,
                                               description="Choose Action Constraint that will be deleted for selected bones.")
    action_mode = bpy.props.EnumProperty(name="Action", items=(("ADD_CONSTRAINT", "Add Constraints", "Add Constraints"),
                                                               ("DELETE_CONSTRAINT", "Delete Constraints",
                                                                "Delete Constraints")),
    ...
    action_frame_start = bpy.props.IntProperty(name="Min Value", default=0,
                                               description="Value where the animations is starting.")
    action_frame_end = bpy.props.IntProperty(name="Max Value", default=10,
                                             description="Value where the animation is ending.")

    prop_min_value = bpy.props.FloatProperty(name="Min Value", default=0.0,
                                             description="That value is used as 0.0 value for the Property.")
    prop_max_value = bpy.props.FloatProperty(name="Max Value", default=1.0,
                                             description="That value is used as 1.0 value for the Property.")
    flip_driver_limits = bpy.props.BoolProperty(name="Flip Driver Limits", default=False,
                                                description="This Bool Property flips the Driver Limits.",
                                                update=driver_limits_flip)
    flip_property_limits = bpy.props.BoolProperty(name="Flip Property Limits", default=False,
                                                  description="This Bool Property flips the Property Limits.",
                                                  update=property_limits_flip)

    set_driver_limit_constraint = bpy.props.BoolProperty(name="Set Driver limit Constraint", default=False,
            ...
            if obj != context.scene.objects.active or len(context.selected_objects) == 1:
                if get_prop_object(self, context, self.prop_data_path, obj) is not None:
                    ...
                    if prop_type in ["MODIFIER_PROPERTY", "OBJECT_CONSTRAINT_PROPERTY"]:
                        ...
                        if self.prop_data_path.rfind("]") == len(
                                self.prop_data_path) - 1:  ### this is used for props of that type: bones["bone_name"]["property_name"]
                ...
                for curve in curves:
                    if curve is not None:
                        ...
                        delete_len = 0
                        for _ in curve.keyframe_points:
                            delete_len += 1
    ...
    def invoke(self, context, event):
        ...
        if wm.clipboard != "":
            if get_prop_object(self, context, wm.clipboard, obj) is not None:
